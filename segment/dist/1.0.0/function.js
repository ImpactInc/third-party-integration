 ...
 * @param {Object} event
 * @param {Object<String, *>} settings
 * @returns {{DeviceModel: *, EventDate: number, ReferringUrl: *, DeviceLocale: string, Latitude: number, CustomProfileId: *, CustomerId: *, AppVer: *, DeviceOs: *, Longitude: number, AppName: *, DeviceMfr: *, AppPackage: *, PageUrl: (*|{}|{enumerable: boolean}|string), ImpactAppId: *, CampaignId: *, CustomerEmail: ({type: string}|{type: string}|{type: string}|string), UserAgent: string | Window._resourceLoader._userAgent | {enumerable: boolean}, DeviceOsVer: *, IpAddress: string, DeviceCarrier: *}}
 * @module mapper
 */
const commonMapper = (event, settings) => {
  return {
    CampaignId: settings.campaignId,
    ImpactAppId: settings.appId,
...
const appMapper = (event, settings) => {
  switch (String(event.context.device.type).toLowerCase()) {
    ...
    case DEVICE_TYPE.ANDROID.toLowerCase():
      return {
        ImpactAppId: settings.androidAppId,
...
const ORDER_KEY_MAPPING = {
  ...
  revenue: 'OrderSubTotalPostDiscount',
...
module.exports = (event) => {
  ...
  for (let i = 0, count = products.length; i < count; ++i) {
    ...
    for (const key in product) {
      /* istanbul ignore if */
      if (!product.hasOwnProperty(key) || !ITEM_KEY_MAPPING[key]) {
        continue;
      }
      productData[(ITEM_KEY_MAPPING[key]) + i] = product[key];
    }
    if (!product.price && product.subtotal) {
      productData[ITEM_KEY_MAPPING.price + i] = product.subtotal;
    }
  }

  if (!productData[ITEM_KEY_MAPPING.price + '0'] && properties.total) {
    productData[ORDER_KEY_MAPPING.revenue] = properties.total;
  }
  ...
  return (event, settings, commonData, eventTypeOverride) => {
    ...
    switch (eventTypeOverride || eventType) {
      case EVENT_TYPE.ORDER_COMPLETE:
        promisedAuxiliaryData = action(event, settings, commonData);
        break;
...
const DEVICE_TYPE = __webpack_require__(/*! ../config */ "./src/config.js").DEVICE_TYPE;
 ...
 *
 * @param {Object} event
 * @param {bject<String, *>} settings
 * @param {bject<String, *>} commonData
 * @returns {Promise<Object<String, *>>}
 * @module track/action
 */
module.exports = (event, settings, commonData) => {
  return new Promise((resolve) => {
    process.nextTick(() => {
      const auxiliaryData = mapProductData(event);
      if (commonData.ImpactAppId) {
        auxiliaryData.MobileApp = true;
        auxiliaryData.PropertyId = String(event.context.device.type).toLowerCase() === DEVICE_TYPE.IOS
          ? settings.iOsAppId
